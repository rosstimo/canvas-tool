#!/bin/bash
# This script fetches a quiz from a Canvas course using the Canvas API.

# Usage/help function
usage() {
  echo "Usage: $0 COURSE_ID QUIZ_ID"
  echo "  COURSE_ID: Canvas course ID (required)"
  echo "  QUIZ_ID: Canvas quiz ID (required)"
  echo "  .env file must set CANVAS_BASE_URL and CANVAS_API_TOKEN"
  echo "Example: $0 5040 12345"
}
...
curl -sS -X GET \
  ...
  "$CANVAS_BASE_URL/api/v1/courses/$COURSE_ID/quizzes/$QUIZ_ID" | jq .
